...
const diff = (left, right, opt = {}) => {
  ...
  let path = '';
  if (opt.path) {
    if (opt.key !== null && typeof opt.path === 'string') {
  ...
  if (dataType === TYPES.OBJECT) {
    ...
    const key = 'id';
    let typesInArray = lArray
      .concat(rArray)
      .map(getType)
      .filter((t) => t !== TYPES.UNKNOWN);
    ...
    Object.keys(arrayResult).forEach((aKey) => {
      arrayResult[aKey].forEach(({ left, right, index }) => {
        switch (aKey) {
          case 'added':
            result.push({
